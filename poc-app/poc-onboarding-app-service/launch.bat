...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "poc-onboarding-app-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name poc-onboarding-app-service apps/poc-onboarding-app-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app poc-onboarding-app-service:1.0-SNAPSHOT
		call oc expose svc/poc-onboarding-app-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd poc-onboarding-app-service
		cd target
