...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "poc-onboarding-app-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name poc-onboarding-app-service apps/poc-onboarding-app-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app poc-onboarding-app-service:1.0-SNAPSHOT
    oc expose svc/poc-onboarding-app-service
else

	echo "Launching the application locally..."
	pattern="poc-onboarding-app-service"
	files=( $pattern )
	cd ${files[0]}
